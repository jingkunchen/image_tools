...
def outPaintingNpy(img_set):
    ...
    for j in range(count):
        ...
        for i in range(0,num[1]):
            ...
            tmp = random.sample(range(10, 30),1)[0]
            img_set_tmp[j,rs_2_w_2[i]:224,rs_2_h[i]:rs_2_h[i]+tmp,:] = img_set[file_num[i],rs_2_w_2[i]:224,rs_2_h[i]:rs_2_h[i]+tmp,:]
        ...
        for i in range(0,num[3]):
            ...
            tmp = random.sample(range(10, 30),1)[0]
            img_set_tmp[j,rs_4_h[i]:rs_4_h[i]+tmp,rs_4_w_2[i]:224,:] = img_set[file_num[i],rs_4_h[i]:rs_4_h[i]+tmp,rs_4_w_2[i]:224,:]
...
if __name__ == "__main__":
    ...
    merge = 1
    if translate:
        img_dir = '/Users/chenjingkun/Documents/data/uiuc_texture_dataset/test'
        img_array = saveNpy(img_dir)
        np.save("uiuc_texture_test.npy",img_array)

    if painting:
        npy_file = "/Users/chenjingkun/Documents/code/texture/uiuc_texture_test.npy"
        img_set = np.load(npy_file)
        new_img_set = inPaintingNpy(img_set)
        np.save("uiuc_texture_inpainting_test.npy",new_img_set)
